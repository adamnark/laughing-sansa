...
public class PollServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        super.init();
        poll = new PollLogic(new String[] {"Jonh", "George", "Paul", "Ringo"});
    }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String sessionChoice = (String)request.getSession(true).getAttribute(CHOICE_PARAMETER_NAME);
...
//        }
        ...
        try {
            ...
            out.println("<link href=\"http://twitter.github.com/bootstrap/assets/css/bootstrap.css\" rel=\"stylesheet\">");
            
            out.println("   <body>");
            out.println("       <div style='margin: 20%'>");

            if (parameterChoice == null && sessionChoice == null) {
                //no user choice and no cookie choice
                //this means the user is accessing the servlet for the first time
                //display the poll input
                writePollForm(out);
            } else if (parameterChoice != null && sessionChoice == null) {
...
//                response.addCookie(new Cookie(CHOICE_PARAMETER_NAME, userChoice));
                request.getSession(true).setAttribute(CHOICE_PARAMETER_NAME, parameterChoice);
                ...
                writeUserChoice(out, sessionChoice.toString());
                writePollResults(out);
            } else if (parameterChoice != null && sessionChoice != null) {
                ...
                writeErrorMessage(out, sessionChoice.toString());
                writePollResults(out); 
            }
            out.println("       </div>");
            out.println("   </body>");
...
//            out.println("<span>" + choice + "</span><span>: " + formatVotes(choiceCount) + "</span><br>");
    ...
    private String formatVotes (int votesCount) {
        return votesCount == 0 ? "no votes" : votesCount == 1 ? "one vote" : votesCount + " votes";
    }
    ...
    private void writeErrorMessage(PrintWriter out, String choice) {
        out.println("<h1 style=\"color: red\">You cannot fill the pole more than 1 time!</h1>");
